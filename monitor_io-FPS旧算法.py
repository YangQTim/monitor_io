...
def plot_io_stats(read_bytes_sec, write_bytes_sec):
    ...
    def autolabel(rects):
        ...
        for rect in rects:
            ...
            ax.annotate(f'{current_height:.1f} b/s',
...
#     """Get the frame statistics using gfxinfo."""
...
#     if start_index == -1:
#         log_message("PROFILEDATA not found.")
#         return

#     # 假设第一行包含列标题
#     header_line = lines[start_index]
#     intended_vsync_index, frame_completed_index = find_indices(header_line)

#     cur_janky_frames = 0.0
#     frame_times = []
#     FPS_frame_times = []
...
#             continue
#         try:
#             # 第intended_vsync_index个时间戳是IntendedVsync, 第frame_completed_index个时间戳是FrameCompleted
#             flags = int(parts[0])
#             start_ns = int(parts[intended_vsync_index])
#             end_ns = int(parts[frame_completed_index])
# 			# 转换为毫秒
#             frame_time_ms = (end_ns - start_ns) / 1000000.0		
			
#             if flags == 0 and frame_time_ms > 16.67:
#                 cur_janky_frames = cur_janky_frames + 1
				
#             if flags == 0 :
#                 frame_times.append(frame_time_ms)
#                 if frame_time_ms < 16.67:
#                     frame_time_ms = 16.67
#                 FPS_frame_times.append(frame_time_ms)
#             else:
#                 log_message("No valid flags.")
#         except ValueError:
#             # 忽略解析错误的行
#             continue
	
#     if frame_times:
#         average_FPS_frame_times = sum(FPS_frame_times) / len(FPS_frame_times)	
#         fps = 1000 / average_FPS_frame_times	
#     else:
#         fps = 0
#         log_message("No valid frame times found.")
		
#     return fps,janky_frames
        ...
        for timestamp in timestamps:
            if timestamp > 16.67:
                ...
                if timestamp % 16.67 == 0:
                    ...
                    vsyncOverTimes += floor(timestamp / 16.67)

        fps = frame_count / (frame_count + vsyncOverTimes) * 60
    return fps,janky_list
...
def monitor_io_and_fps(package_name,interval=0.5):
    ...
    while True:
        ...
        interval_time = (current_timer - prev_timer) *1000
        read_bytes_sec = (read_bytes_diff / interval_time)
        write_bytes_sec = (write_bytes_diff / interval_time)
        prev_io_stats = current_io_stats
        prev_timer = current_timer
        log_message(f"Read: {read_bytes_sec} bytes/s, Write: {write_bytes_sec} bytes/s")
...
if __name__ == "__main__":
    ...
    read_bytes_sec = write_bytes_sec = fps = package_cpu_usage = total_cpu_usage = 0 # 初始化绘图全局变量
    global stop_threads 
    stop_threads = False
    global monitor_thread 
    monitor_thread = None
